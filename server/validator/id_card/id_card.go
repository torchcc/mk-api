...
// Get information from id_card number. Birthday, gender, province mask.
func GetCitizenNoInfo(citizenNo []byte) (birthday int64, isMale bool, addrMask int, err error) {
	...
	if !IsValidCitizenNo(&citizenNo) {
		err = errors.New("invalid id_card number")
		return
	}
